(function() {
            ...
            if (viewType === 'roster') {
                        ...
                        <div style="display:flex; gap:15px;">
                            ...
                            <div style="flex:1;">
                                <div style="display:flex; justify-content:space-between;">
                                    <div style="color:#1a1a1a; font-weight:bold; font-size:14px;">${name}</div>
                                ...
                                <div class="cte-rpg-stat-row">
                                    <div class="cte-rpg-stat-bar-container">
                                        ...
                                        <div class="bar-bg">
                                            <div class="bar-fill" style="width:${stats.vocal}%; background:#1a1a1a;"></div>
                                        </div>
                                    </div>
                                    <div class="cte-rpg-stat-bar-container">
                                        <div class="label" style="display:flex; justify-content:space-between;">
                                        ...
                                        <div class="bar-bg">
                                            <div class="bar-fill" style="width:${stats.dance}%; background:#1a1a1a;"></div>
                                        </div>
                                    </div>
                                </div>
                                <div class="cte-rpg-stat-row" style="margin-top: 5px;">
                                    <div class="cte-rpg-stat-bar-container">
                                        <div class="label" style="display:flex; justify-content:space-between;">
                                            <span>欲望</span> <span style="color:#a84444;">${profile.status.desire}%</span>
                                        </div>
                                        <div class="bar-bg">
                                            <div class="bar-fill" style="width:${profile.status.desire}%; background:#a84444;"></div>
                                        </div>
                                    </div>
                                    <div class="cte-rpg-stat-bar-container">
                                        <div class="label" style="display:flex; justify-content:space-between;">
                                            <span>好感</span> <span style="color:#a84444;">${profile.status.affection}%</span>
                                        </div>
                                        <div class="bar-bg">
                                            <div class="bar-fill" style="width:${profile.status.affection}%; background:#a84444;"></div>
                                        </div>
                                    </div>
                                </div>
                                ${warningHtml}
                ...
                htmlContent = `
                    <div style="display:grid; grid-template-columns: 1fr 1fr; gap:20px;">
                        <div class="cte-rpg-card">
                            <h3 style="color:#1a1a1a; font-size:14px; margin-top:0;">近期通告</h3>
                            <ul style="list-style:none; padding:0; font-size:12px; color:#555;">
                                <li style="padding:5px 0; border-bottom:1px solid rgba(0,0,0,0.1);">京港电视台专访 <span style="float:right; color:#1a1a1a; font-weight:bold;">完成</span></li>
                                <li style="padding:5px 0; border-bottom:1px solid rgba(0,0,0,0.1);">新专辑《NEON》筹备 <span style="float:right; color:#888;">进行中</span></li>
                            </ul>
                        </div>
                    </div>
                `;
    ...
    window.CTEMap.switchView = function(viewName, btn) {
        ...
        if (viewName === 'schedule') window.CTEMap.refreshSchedule();
        if (viewName === 'manager') {
        ...
        if (viewName === 'heartbeat') window.CTEMap.Heartbeat.renderGrid();
    };
    ...
    async function initializeExtension() {
        ...
        const panelHTML = `
            <div id="cte-toggle-btn" title="点击打开 / 长按拖动" 
                 style="position:fixed; top:130px; left:10px; z-index:9000; width:40px; height:40px; background:#a84444; border-radius:50%; display:flex; justify-content:center; align-items:center; cursor:move; box-shadow:0 4px 10px rgba(0,0,0,0.3); color:#fff; font-size:20px;">
                🗺️
            </div>
            <div id="cte-map-panel" style="display:none;">
                <div id="cte-drag-handle">
                    ...
                    <div class="cte-nav-group">
                        ...
                        <button class="cte-nav-btn" style="color: #a84444;" onclick="window.CTEMap.switchView('heartbeat', this)">♥</button>
        ...
        if ($.fn.draggable) {
            $('#cte-map-panel').draggable({ handle: '#cte-drag-handle', containment: 'window' });
    ...
    window.CTEMap.initNationalMap = function() {
        ...
        window.CTEMap.nationalCities.forEach(city => {
            ...
            cityEl.onmousedown = function(e) {
                ...
                initialLeft = cityEl.offsetLeft; initialTop = cityEl.offsetTop;
                ...
                document.onmousemove = function(e) {
                    ...
                    cityEl.style.left = newLeft + 'px'; cityEl.style.top = newTop + 'px';
                };

                document.onmouseup = function() {
                    isDragging = false; document.onmousemove = null; document.onmouseup = null;
                    if (!hasMoved) {
                        if (city.isReturn) window.CTEMap.switchView('map');
                        else {
                            let html = `<h2><i class="fa-solid fa-scroll"></i> ${city.name} - 情报简报</h2><ul><li>${city.info}</li></ul>`;
                            html += `<div style="text-align:center; margin-top:15px; border-top:1px dashed #666; padding-top:10px;"><button class="cte-btn" onclick="window.CTEMap.openTravelMenu('${city.name}')" style="width:80%;">🚀 前往 ${city.name}</button></div>`;
    ...
    window.CTEMap.openTravelMenu = function(destination) {
        ...
        if (window.CTEMap.isSelectingForSchedule) {
            box.find('.travel-options').html(`
                <div style="text-align:center; color:#555; margin-bottom:15px; font-size:14px; border-bottom:1px solid #ddd; padding-bottom:10px;">
                    正在执行行程：<br><span style="color:#a84444; font-weight:bold;">${window.CTEMap.currentScheduleItem}</span>
                </div>
                <div style="margin-bottom: 15px; border-bottom: 1px solid #ddd; padding-bottom: 10px;">
                    <div style="display:flex; align-items:center; justify-content:space-between; margin-bottom:8px;">
                        <span style="color:#888; font-size:13px;">是否遇见NPC？</span>
                        <div>
                            <button id="btn-npc-yes" class="cte-btn" style="font-size:12px; padding:2px 8px; margin-right:5px; border-color:#ccc;" onclick="window.CTEMap.toggleNPC(true, '${defaultNPC}')">是</button>
                            <button id="btn-npc-no" class="cte-btn" style="font-size:12px; padding:2px 8px; background:#a84444; color:white;" onclick="window.CTEMap.toggleNPC(false)">否</button>
                        </div>
                    </div>
                    <input type="text" id="npc-input" class="travel-input" style="display:none; font-size:13px; margin-bottom:0;" placeholder="请输入遇见的人" value="${defaultNPC}">
                </div>
                <button class="cte-btn primary" onclick="window.CTEMap.finalizeScheduleExecution()" style="width:100%;">✅ 确认执行</button>
                <button class="cte-btn" style="margin-top: 10px; width:100%;" onclick="window.CTEMap.closeTravelMenu()">取消</button>
            `);
        } else {
            box.find('.travel-options').html(`
                <div style="margin-bottom: 15px; border-bottom: 1px solid #ddd; padding-bottom: 10px;">
                ...
                <button class="cte-btn" style="margin-top: 10px;" onclick="window.CTEMap.closeTravelMenu()">关闭</button>
            `);
        }
    ...
    window.CTEMap.toggleNPC = function(enable, defaultText) {
        ...
        if (enable) {
            ...
            btnYes.style.background = '#a84444'; btnYes.style.color = 'white'; btnYes.style.borderColor = '#a84444';
            btnNo.style.background = 'transparent'; btnNo.style.color = '#888'; btnNo.style.borderColor = '#ccc';
        } else {
            input.style.display = 'none';
            btnNo.style.background = '#a84444'; btnNo.style.color = 'white'; btnNo.style.borderColor = '#a84444';
            btnYes.style.background = 'transparent'; btnYes.style.color = '#888'; btnYes.style.borderColor = '#ccc';
        }
    };
    ...
    window.CTEMap.showCompanionInput = function() {
        $('#travel-menu-overlay .travel-options').html(`
            ...
            <button class="cte-btn primary" style="width:100%;" onclick="window.CTEMap.validateAndShowActivities()">🤝 一起前往</button>
            <button class="cte-btn" style="margin-top: 10px; width:100%;" onclick="window.CTEMap.openTravelMenu('${window.CTEMap.currentDestination}')">返回</button>
        `);
    };
    ...
    window.CTEMap.showActivityMenu = function() {
        ...
        $('#travel-menu-overlay .travel-options').html(`
            <p style="color: #888; margin: 0 0 10px 0;">去做什么？</p>
            ...
            <div style="border-top: 1px solid #ddd; padding-top: 10px; width: 100%;">
                ...
                <button class="cte-btn primary" style="width:100%;" onclick="window.CTEMap.finalizeTravel(null)">🚀 确认出发</button>
            </div>
            <button class="cte-btn" style="margin-top: 10px; width:100%;" onclick="window.CTEMap.showCompanionInput()">返回上一步</button>
        `);
    };
            ...
            if (roomName === '你') {
                ...
                contentHTML = `
                    <div class="character-room-detail">
                        ...
                        <div class="action-buttons"><button class="cte-btn primary" onclick="window.CTEMap.openTravelMenu('你的房间')">🚀 前往</button><button class="sub-item-btn" id="temp-back-btn">[ < 返回上一级 ]</button></div>
                    </div>`;
            } else {
                contentHTML = `
                    <div class="character-room-detail">
                        ...
                        <div class="room-description"><p>${desc}</p></div>
                        <div class="action-buttons"><button class="cte-btn primary" onclick="window.CTEMap.openTravelMenu('${roomName}的房间')">🚀 前往</button><button class="sub-item-btn" id="temp-back-btn">[ < 返回上一级 ]</button></div>
                    </div>`;
            }
        } else {
            contentHTML = `
                <div style="display: flex; flex-direction: column; align-items: center; gap: 15px; width: 100%;">
                    ...
                    <button class="cte-btn primary" onclick="window.CTEMap.openTravelMenu('${roomName}')">🚀 前往</button>
    ...
    window.CTEMap.openRooftopMenu = function() {
        ...
        contentEl.innerHTML = `
            <div style="display: flex; flex-direction: column; align-items: center; gap: 15px; width: 100%;">
                ...
                <button class="cte-btn primary" onclick="window.CTEMap.openTravelMenu('天台花园酒吧')">🚀 前往</button>
            </div>`;
    };
